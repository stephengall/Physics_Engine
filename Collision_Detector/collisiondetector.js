class CollisionDetector {
    ...
    evaluate(rec1, rec2) {
        //for every vertex in rec1 and rec2, evaluate perpVec.x * vertex.x + perpVec.y * vertex.y
...
//creating vector between centres of shapes
    ...
    moveAlongVector() {
        ...
        var collArm1 = createVector();
        var collArm2 = createVector();
        var centre1 = this.rec1.returnCentres();
        var centre2 = this.rec2.returnCentres();

        // collArm1 = collisionPoints[0].sub(centre1);
        // collArm2 = collisionPoints[0].sub(centre2);

        // var angularVel1 = this.rec1.vel.add(this.rec1.rotVel.cross(collArm1));
        // var angularVel2 = this.rec2.vel.add(this.rec2.rotVel.cross(collArm2));

        // var relativeVelocity = angularVel2.sub(angularVel2);
        // var restitution = min(this.rec1.restitution, this.rec2.restitution);
        // var j = -(1 + restitution) * (relativeVelocity.dot(this.minDir));
        // var norm1 = collArm1.cross(this.minDir);
        // var norm2 = collArm2.cross(this.minDir);

        // var den = (1 / this.rec1.mass + 1 / this.rec2.mass) + ((norm1.mult(1 / this.rec1.intertia).mult(norm1)).add((norm2).mult(1 / this.rec2.intertia).mult(norm2)));
        // j /= den;
